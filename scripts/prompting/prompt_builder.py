...
class TextFilePromptBuilder(PromptBuilder):
    ...
    def build(self, query: str, context: str) -> str:
        ...
        This method dispatches to a style-specific build method
        (e.g., `_build_prompt` or `_build_prompt_references`) based on the
        `self.style` attribute set during initialization.

        Args:
            query (str): The user's query.
            context (str): The contextual information (e.g., retrieved text chunks)
        ...
        """
        if self.style == "references":
            return self._build_prompt_references(query, context)
        # Default to the standard prompt style if 'style' is not 'references' or is unknown
        return self._build_prompt(query, context)
...
Answer:"""
    ...
    def _build_prompt_references(self, query: str, context: str) -> str:
        """
